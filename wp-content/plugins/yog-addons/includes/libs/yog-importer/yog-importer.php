...
function yog_is_str_contain( $search, $in ) {
    // if in string
...
// Don't duplicate me!
if ( !class_exists( 'Yog_Theme_Importer' ) ) {
  ...
  class Yog_Theme_Importer {
    ...
    public function __construct( $key ) {
      ...
      $this->theme_options_file = get_template_directory(). "/theme/demo/{$key}/theme-options.json";
      $this->widgets            = get_template_directory(). "/theme/demo/{$key}/widgets.txt";
      $this->content_demo       = get_template_directory(). "/theme/demo/{$key}/content.xml";
    ...
    public function set_demo_data( $file ) {
      ...
      if ( !class_exists( 'WP_Import' ) ) {
        ...
        $class_wp_import = ABSPATH . 'wp-content/plugins/yog-addons/includes/libs/yog-importer/wordpress-importer/wordpress-importer.php';
    ...
    public function set_demo_menus() {
        ...
        $yog_menus = get_theme_support( 'yog-menu' );
        
        
        // If there is no menus, return.
        if ( ! is_array( $yog_menus[0] ) ) return;
        ...
        if( !empty( $menus ) && !empty( $yog_menus ) ) {

            foreach( $yog_menus[0] as $location => $search ) {

                foreach( $menus as $menu ) {

                    if( is_object( $menu ) && yog_is_str_contain( $search, $menu->name ) ) {
    ...
    public function set_demo_theme_options( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Theme options Import file could not be found. Please try again.', 'flipmart' ),
          '',
      ...
      if ( !empty( $data ) || is_array( $data ) ) {
          ...
          update_option( yog()->get_option_name(), $data );
      }
    ...
    function process_widget_import_file( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Widget Import file could not be found. Please try again.', 'flipmart' ),
          '',
          array( 'back_link' => true )
        );
      }
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
          ...
          $sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'flipmart' );
        }
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
            ...
            $widget_message = __( 'Site does not support widget', 'flipmart' ); // explain why widget not imported
          }
          ...
          if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
            ...
            foreach ( $single_widget_instances as $check_id => $check_widget ) {
              ...
              if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
                ...
                $widget_message = __( 'Widget already exists', 'flipmart' ); // explain why widget not imported
          ...
          if ( ! $fail ) {
            ...
            if ( $sidebar_available ) {
              ...
              $widget_message = __( 'Imported', 'flipmart' );
            } else {
              $widget_message_type = 'warning';
              $widget_message = __( 'Imported to Inactive', 'flipmart' );
            }
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'flipmart' ); // show "No Title" if widget instance is untitled
...
/**
 *
 * @method yog_install_dummy_data
 * @param  [type]                              [description]
 * @return [type]                              [description]
 */
function yog_install_dummy_data() {
    
    if( !current_user_can( 'manage_options' ) ) return;
    if( isset( $_GET['import-done'] ) && $_GET['import-done'] ) {
        add_action( 'yog_before_admin_panel', 'yog_import_demo_message' );
        function yog_import_demo_message() {
        	echo '<div class="alert success">'. __( 'Sucessfully imported demo data!', 'flipmart' ) .'</div>';   
        }
        
        return;
    }
    
    if( !isset( $_GET['yog-import-demo'] ) ) return;
    
    // Doing import
    $importer = new Yog_Theme_Importer( $_GET['yog-import-demo'] );

    wp_safe_redirect( admin_url( 'admin.php?page=yog&import-done=1') );
}
add_action( 'admin_init', 'yog_install_dummy_data', 1 );